...
Public Class OrderId
    ...
    Private Sub listaOrders_SelectedIndexChanged(sender As Object, e As EventArgs) Handles listaOrders.SelectedIndexChanged
        ...
        Dim sentencia2 As String = "SELECT SUM(UnitPrice * Quantity * (1-Discount)) WHERE OrderID = " & Convert.ToInt32(listaOrders.SelectedValue.ToString)
        Dim dr As OleDbDataReader
        Dim adap1 As New OleDbDataAdapter(sentencia, conexion)
        Dim comando As New OleDbCommand(sentencia, conexion)
        Dim das1 As New DataSet
        ...
        Try
            ...
            lblPrecioTotal.Text = das2.Tables("tabla").Rows.Item(0).Item(0).ToString() & "€"
        Catch ex As Exception
        Finally
            conexion.Close()
        End Try
